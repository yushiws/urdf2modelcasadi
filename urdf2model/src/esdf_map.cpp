...
bool fiesta::ESDFMap::PosInMap(Eigen::Vector2d pos) {
#ifdef HASH_TABLE
    return true;
#else
    if (pos(0) < min_range_(0) || pos(1) < min_range_(1)) {
        //    cout << "less than min range!\t" << pos(0) << ' ' << pos(1) << ' '
        //    << pos(2) << endl;
        return false;
    }

    if (pos(0) > max_range_(0) || pos(1) > max_range_(1)) {
        //    cout << "larger than min range!\t" << pos(0) << ' ' << pos(1) << '
        //    ' << pos(2) << endl;
        return false;
    }
    return true;
...
bool fiesta::ESDFMap::VoxInRange(Eigen::Vector2i vox, bool current_vec) {
    if (current_vec)
...
void fiesta::ESDFMap::Pos2Vox(Eigen::Vector2d pos, Eigen::Vector2i &vox) {
    for (int i = 0; i < 2; ++i)
        vox(i) = floor((pos(i) - origin_(i)) / resolution_);
}

void fiesta::ESDFMap::Vox2Pos(Eigen::Vector2i vox, Eigen::Vector2d &pos) {
    for (int i = 0; i < 2; ++i)
        pos(i) = (vox(i) + 0.5) * resolution_ + origin_(i);
}

int fiesta::ESDFMap::Vox2Idx(Eigen::Vector2i vox) {
#ifdef HASH_TABLE
    if (vox(0) == undefined_)
        return reserved_idx_4_undefined_;
    return FindAndInsert(Eigen::Vector2i(vox(0), vox(1), vox(2)));
#else
    if (vox(0) == undefined_)
        return reserved_idx_4_undefined_;
    return vox(0) * grid_size_(1) + vox(1);
#endif
}

int fiesta::ESDFMap::Vox2Idx(Eigen::Vector2i vox, int sub_sampling_factor) {
#ifdef HASH_TABLE
    ...
    if (vox(0) == undefined_)
        return reserved_idx_4_undefined_;
    return vox(0) * grid_size_(1) / sub_sampling_factor / sub_sampling_factor +
           vox(1) / sub_sampling_factor;
#endif
}

Eigen::Vector2i fiesta::ESDFMap::Idx2Vox(int idx) {
#ifdef HASH_TABLE
    return vox_buffer_[idx];
#else
    return Eigen::Vector2i(idx / grid_size_(1), idx % grid_size_(1));
#endif
}

// endregion

double fiesta::ESDFMap::Dist(Eigen::Vector2i a, Eigen::Vector2i b) {
...
fiesta::ESDFMap::ESDFMap(Eigen::Vector2d origin, double resolution_,
                         int reserve_size)
    ...
    std::fill(closest_obstacle_.begin(), closest_obstacle_.end(),
              Eigen::Vector2i(undefined_, undefined_, undefined_));
    std::fill(vox_buffer_.begin(), vox_buffer_.end(),
              Eigen::Vector2i(undefined_, undefined_, undefined_));
...
fiesta::ESDFMap::ESDFMap(Eigen::Vector2d origin, double resolution_,
    ...
    for (int i = 0; i < 2; ++i) grid_size_(i) = ceil(map_size(i) / resolution_);
    ...
    grid_total_size_ = grid_size_(0) * grid_size_(1);
    reserved_idx_4_undefined_ = grid_total_size_;
    SetOriginalRange();

    cout << "grid total size: " << grid_total_size_ << endl;
    ...
    std::fill(distance_buffer_.begin(), distance_buffer_.end(),
              (double)undefined_);
    //    std::fill(distanceBufferNegative.begin(),
    //    distanceBufferNegative.end(), (double) undefined_);
...
#ifdef PROBABILISTIC
void fiesta::ESDFMap::SetParameters(double p_hit, double p_miss, double p_min,
                                    double p_max, double p_occ) {
...
bool fiesta::ESDFMap::UpdateOccupancy(bool global_map) {
    ...
    while (!occupancy_queue_.empty()) {
        ...
        double log_odds_update =
            (num_hit_[idx] >= num_miss_[idx] - num_hit_[idx] ? prob_hit_log_
                                                             : prob_miss_log_);
...
void fiesta::ESDFMap::UpdateESDF() {
    ...
    //    startTime = clock();
    //    UpdateOccupancy();
    cout << "Insert " << insert_queue_.size() << "\tDelete "
         << delete_queue_.size() << endl;
    ...
    while (!update_queue_.empty()) {
        ...
        for (int i = 0; i < num_dirs_; i++) {
            Eigen::Vector2i new_pos = xx.point_ + dirs_[i];
            if (VoxInRange(new_pos)) {
                int new_pos_idx = Vox2Idx(new_pos);
    ...
    total_time_ += times;
    cout << "Expanding " << times << " nodes, with change_num = " << change_num
         << ", accumulator = " << total_time_ << endl;
    //    endTime = clock();
    //    cout << "Totle Time : " <<(double)(endTime - startTime) /
    //    CLOCKS_PER_SEC << "s" << endl;
}

int fiesta::ESDFMap::SetOccupancy(Eigen::Vector2d pos, int occ) {
    if (occ != 1 && occ != 0) {
...
int fiesta::ESDFMap::SetOccupancy(Eigen::Vector2i vox, int occ) {
...
int fiesta::ESDFMap::GetOccupancy(Eigen::Vector2d pos) {
    if (!PosInMap(pos))
        return undefined_;
...
int fiesta::ESDFMap::GetOccupancy(Eigen::Vector2i pos_id) {
    // TODO: no boundary check
    return Exist(Vox2Idx(std::move(pos_id)));
}

double fiesta::ESDFMap::GetDistance(Eigen::Vector2d pos) {
    if (!PosInMap(pos))
        return undefined_;
...
double fiesta::ESDFMap::GetDistance(Eigen::Vector2i vox) {
    return distance_buffer_[Vox2Idx(vox)] < 0 ? infinity_
                                              : distance_buffer_[Vox2Idx(vox)];
}

double fiesta::ESDFMap::GetDistWithGradTrilinear(Eigen::Vector2d pos,
                                                 Eigen::Vector2d &grad) {
    if (!PosInMap(pos))
        return -1;
    ...
    Eigen::Vector2d pos_m = pos - 0.5 * resolution_ * Eigen::Vector2d::Ones();
...
void fiesta::ESDFMap::GetPointCloud(sensor_msgs::PointCloud &m,
                                    int vis_lower_bound, int vis_upper_bound) {
    m.header.frame_id = "world";
    m.points.clear();
#ifdef HASH_TABLE
    for (int i = 1; i < count; i++) {
        if (!Exist(Vox2Idx(vox_buffer_[i])) ||
            vox_buffer_[i].z() < vis_lower_bound ||
            vox_buffer_[i].z() > vis_upper_bound ||
            vox_buffer_[i].x() < min_vec_(0) ||
            vox_buffer_[i].x() > max_vec_(0) ||
            vox_buffer_[i].y() < min_vec_(1) ||
            vox_buffer_[i].y() > max_vec_(1))
            continue;

        Eigen::Vector2d pos;
        Vox2Pos(Eigen::Vector2i(vox_buffer_[i]), pos);
    ...
    for (int x = min_vec_(0); x <= max_vec_(0); ++x)
        for (int y = min_vec_(1); y <= max_vec_(1); ++y)
            for (int z = min_vec_(2); z <= max_vec_(2); ++z) {
                if (!Exist(Vox2Idx(Eigen::Vector2i(x, y, z))) ||
                    z < vis_lower_bound || z > vis_upper_bound)
                    continue;

                Eigen::Vector2d pos;
                Vox2Pos(Eigen::Vector2i(x, y, z), pos);
...
void fiesta::ESDFMap::GetSliceMarker(visualization_msgs::Marker &m, int slice,
    ...
    for (int i = 1; i < count; i++) {
        ...
        if (vox_buffer_[i].z() != slice || distance_buffer_[idx] < 0 ||
            distance_buffer_[idx] >= infinity_ ||
            vox_buffer_[i].x() < min_vec_(0) ||
            vox_buffer_[i].x() > max_vec_(0) ||
            vox_buffer_[i].y() < min_vec_(1) ||
        ...
        p.y = pos(1);
        p.z = pos(2);
        c = RainbowColorMap(distance_buffer_[idx] <= max_dist
                                ? distance_buffer_[idx] / max_dist
                                : 1);
        m.points.push_back(p);
    ...
    for (int x = min_vec_(0); x <= max_vec_(0); ++x)
        for (int y = min_vec_(1); y <= max_vec_(1); ++y) {
            int z = slice;
            Eigen::Vector2i vox = Eigen::Vector2i(x, y, z);
...
void fiesta::ESDFMap::IncreaseCapacity(int &old_size, int new_size) {
    ...
    std::fill(distance_buffer_.begin() + old_size, distance_buffer_.end(),
              (double)undefined_);
    ...
    std::fill(closest_obstacle_.begin() + old_size, closest_obstacle_.end(),
              Eigen::Vector2i(undefined_, undefined_, undefined_));
    std::fill(vox_buffer_.begin() + old_size, vox_buffer_.end(),
              Eigen::Vector2i(undefined_, undefined_, undefined_));
...
int fiesta::ESDFMap::FindAndInsert(Eigen::Vector2i hash_key) {
#ifdef BLOCK
    if (count + block_size_ > reserve_size_)
        IncreaseCapacity(reserve_size_, reserve_size_ * 2);
#ifdef BITWISE
    int idx_in_block =
        ((hash_key.x() & (block_ - 1)) << block_bit_ << block_bit_) +
        ((hash_key.y() & (block_ - 1)) << block_bit_) +
        (hash_key.z() & (block_ - 1));
    Eigen::Vector2i block_id =
        hash_key.unaryExpr([&](const int x) { return x >> block_bit_; });
#else
    int idx_in_block =
        (hash_key.x() % block_ + block_) % block_ * block_ * block_ +
        (hash_key.y() % block_ + block_) % block_ * block_ +
        (hash_key.z() % block_ + block_) % block_;
    Eigen::Vector2i block_id = hash_key.unaryExpr(
        [&](const int x) { return x / block_ - (x % block_ < 0); });
#endif
...
#ifdef BITWISE
        block_id =
            block_id.unaryExpr([&](const int x) { return x << block_bit_; });
#else
        ...
        for (int i = 0; i < block_; i++)
            for (int j = 0; j < block_; j++)
                for (int k = 0; k < block_; k++) {
                    vox_buffer_[count++] = block_id + Eigen::Vector2i(i, j, k);
                }
...
void fiesta::ESDFMap::SetUpdateRange(Eigen::Vector2d min_pos,
                                     Eigen::Vector2d max_pos, bool new_vec) {
#ifndef HASH_TABLE
    ...
    Pos2Vox(max_pos - Eigen::Vector2d(resolution_ / 2, resolution_ / 2),
            max_vec_);
}
...
#else
    min_vec_ << 0, 0;
    max_vec_ << grid_size_(0) - 1, grid_size_(1) - 1;
    last_min_vec_ = min_vec_;
    last_max_vec_ = max_vec_;
#endif
}
...
void fiesta::ESDFMap::SetAway(Eigen::Vector2i left, Eigen::Vector2i right) {
    for (int i = left(0); i <= right(0); i++)
        for (int j = left(1); j <= right(1); j++)
            occupancy_buffer_[Vox2Idx(Eigen::Vector2i(i, j))] |= 2;
}
void fiesta::ESDFMap::SetBack() { SetBack(min_vec_, max_vec_); }
void fiesta::ESDFMap::SetBack(Eigen::Vector2i left, Eigen::Vector2i right) {
    for (int i = left(0); i <= right(0); i++)
        for (int j = left(1); j <= right(1); j++)
            if (occupancy_buffer_[Vox2Idx(Eigen::Vector2i(i, j))] >= 2)
                SetOccupancy(Eigen::Vector2i(i, j), 0);
}
#endif
...
bool fiesta::ESDFMap::CheckConsistency() {
    ...
    for (int ii = 1; ii < count; ii++) {
        ...
        if (prev_[vox] != undefined_ && next_[prev_[vox]] != vox ||
            ...
            std::cout << vox_buffer_[ii](0) << ' ' << vox_buffer_[ii](1) << ' '
                      << vox_buffer_[ii](2) << ' ' << vox << std::endl;
            std::cout << prev_[vox] << ' ' << next_[prev_[vox]] << ' '
                      << next_[vox] << ' ' << prev_[next_[vox]] << std::endl;
            std::cout << Vox2Idx(closest_obstacle_[next_[prev_[vox]]]) << ' '
                      << Vox2Idx(closest_obstacle_[vox]) << std::endl;
            std::cout << Vox2Idx(closest_obstacle_[prev_[vox]]) << ' '
                      << Vox2Idx(closest_obstacle_[next_[vox]]) << std::endl;
    ...
    for (int x = 0; x < grid_size_(0); ++x)
        for (int y = 0; y < grid_size_(1); ++y)
            for (int z = 0; z < grid_size_(2); ++z) {
                int vox = Vox2Idx(Eigen::Vector2i(x, y, z));
                if (prev_[vox] != undefined_ && next_[prev_[vox]] != vox ||
                    next_[vox] != undefined_ && prev_[next_[vox]] != vox) {
                    std::cout << x << ' ' << y << ' ' << z << ' ' << vox
                              << std::endl;
                    std::cout << prev_[vox] << ' ' << next_[prev_[vox]] << ' '
                              << next_[vox] << ' ' << prev_[next_[vox]]
                              << std::endl;
                    std::cout << Vox2Idx(closest_obstacle_[next_[prev_[vox]]])
                              << ' ' << Vox2Idx(closest_obstacle_[vox])
                              << std::endl;
                    std::cout << Vox2Idx(closest_obstacle_[prev_[vox]]) << ' '
                              << Vox2Idx(closest_obstacle_[next_[vox]])
                              << std::endl;
...
bool fiesta::ESDFMap::CheckWithGroundTruth() {
#ifdef HASH_TABLE
    Eigen::Vector2i ma = Eigen::Vector2i(0, 0, 0),
                    mi = Eigen::Vector2i(0, 0, 0);
    //        ESDFMap *esdf_map_ = new ESDFMap(Eigen::Vector2d(0, 0, 0),
    //        resolution, 10000000);
    ...
    for (int ii = 1; ii < count; ii++) {
        if (distance_buffer_[ii] >= 0 && distance_buffer_[ii] < infinity_) {
            kdtree.nearestKSearch(
                pcl::PointXYZ(vox_buffer_[ii](0), vox_buffer_[ii](1),
                              vox_buffer_[ii](2)),
            ...
            //                    std::cout << vox_buffer_[ii](0) << '\t' <<
            //                    vox_buffer_[ii](1) << '\t' <<
            //                    vox_buffer_[ii](2) << '\n'; std::cout <<
            //                    cloud_->points[pointIdxNKNSearch[0]].x << '\t'
            //                              <<
    ...
    for (int x = 0; x < grid_size_(0); ++x)
        for (int y = 0; y < grid_size_(1); ++y)
            for (int z = 0; z < grid_size_(2); ++z) {
                int ii = Vox2Idx(Eigen::Vector2i(x, y, z));
                if (distance_buffer_[ii] >= 0 &&
                    distance_buffer_[ii] < infinity_) {
                    kdtree.nearestKSearch(pcl::PointXYZ(x, y, z), 1,
                                          pointIdxNKNSearch,
                                          pointNKNSquaredDistance);
                    double tmp = sqrt(pointNKNSquaredDistance[0]) * resolution_;
                    //                if (fabs(distance_buffer_[ii] - tmp) >
                    //                1e-3) {
                    //                    std::cout << vox_buffer_[ii](0) <<
                    //                    '\t' << vox_buffer_[ii](1) << '\t' <<
                    //                    vox_buffer_[ii](2) << '\n'; std::cout
                    ...
                    ems1 += distance_buffer_[ii] - tmp;
                    ems2 += (distance_buffer_[ii] - tmp) *
                            (distance_buffer_[ii] - tmp);
                    cnt2++;
                    max1 = std::max(distance_buffer_[ii] - tmp, max1);
                    //                }
                }
